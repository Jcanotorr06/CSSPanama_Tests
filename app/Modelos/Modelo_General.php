<?php
    require_once __DIR__.'/../Db/ConexionDB.php';//Se importa la clase de conexion a la base de datos
    ...
    class Modelo_General{
        ...
        public function agendarCita($data){
            if(empty($data['id_usuario']) || empty($data['id_medico']) || empty($data['fecha']) || empty($data['hora'])){
                return false;
            }
        ...
        public function reprogramarCita($data){
            if(empty($data['id_cita']) || empty($data['id_medico']) || empty($data['fecha']) || empty($data['hora'])){
                return false;
            }
            if($this->validarCitaDisponible($data)){
        ...
        public function BuscarEmailCita($id_cita){
            $res = $this->db->query("Select email from usuarios Join citas as cit on cit.id_usuario = usuarios.id Where cit.id = $id_cita");
            if($res->num_rows > 0){
                while($x = $res->fetch_assoc()){
                    $usuario = $x;
                }
