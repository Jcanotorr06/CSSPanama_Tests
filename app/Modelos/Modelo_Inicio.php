<?php
    ...
    class Modelo_Inicio{//Modelo de Inicio que interactua con la base de datos
        ...
        public function registro($data){
            ...
            if(empty($data['nombre']) || empty($data['apellido']) || empty($data['cedula']) || empty($data['email']) || empty($data['contraseña'])){
                return false;
            }

            //Valida que el correo siga el patron correcto
            if(!preg_grep("/[a-z]{1,}@{1}[a-z]{1,}/", [$data['email']])){
                return false;
            }

            //Valida que la cedula siga el patron correcto
            if(!preg_grep("/([1][0]|\d{1})-\d{1,4}-\d{1,5}/", [$data['cedula']])){
                return false;
            }
            $data['contraseña'] = password_hash($data['contraseña'], PASSWORD_DEFAULT);//Se encripta la contraseña antes de almacenarla
            //Ejecuta el procedimiento almacenado crear_usuario para insertar un nuevp usuario a la BD
